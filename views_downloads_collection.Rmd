...
```{r total downloads on pps by minutes results}
...
```{python pull total downloads on pps by second}
pp = r.overall_downloads_bysecond_pull
...
# set up empty dataframe with the right column names
overall_downloads_persecond_df = pd.DataFrame(columns=['guid', 'date', 'download_count'])
empty_lists = 0
calls = 1
...
for i in range(len(pp.guid)):
  ...
  # make call using query & payload from above
  res = requests.post(post_url, headers=headers, json=payload)
  pp_downloads_second = res.json()['aggregations']['downloads_timeframe']['downloads_per_second']['buckets']
  ...
  if calls % 100 == 0:
    ...
    if len(pp_downloads_second) > 0: # only parse query output if there were actually views/downloads to parse
    
      # loop through each person in output
      for x in range(len(pp_downloads_second)):
      
            # append new row with info for the preprint, the user, the timestamp, and the action count
        new_row = [{'guid':guid, 'date':pp_downloads_second[x]['key_as_string'],'download_count': pp_downloads_second[x]['doc_count']}]
    
        overall_downloads_persecond_df = overall_downloads_persecond_df.append(new_row, ignore_index = True)
...
```{r}
overall_downloads_by_second_results <- py$overall_downloads_persecond_df

overall_downloads_by_second_results <- overall_downloads_by_second_results %>%
                                        as_tibble() %>% unnest(download_count)

write_csv(overall_downloads_by_second_results, 'overall_downloads_by_second_results.csv')
```
...
```{python user_ids for all downloads pull}
import requests
import pandas as pd
...
```{r downloads by second no sloan_id}
downloads_bysecond_nosloanid <- py$downloads_bysecond_nosloanid %>%
                                      as_tibble() %>%
                                      unnest(download_count)

write_csv(downloads_bysecond_nosloanid, 'downloads_bysecond_nosloanid.csv')
```

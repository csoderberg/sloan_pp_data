...
overall_downloads_bysecond_pull <- overall_downloads_by_hour_results %>%
                                      mutate(date = ymd_hms(date)) %>%
                                      filter(download_count > 0) %>%
                                      rename(gte = date) %>%
                                      mutate(lt = gte + hours(1),
...
for i in range(len(pp.guid)):
  ...
  # make call using query & payload from above
  res = requests.post(post_url, headers=headers, json=payload)
  pp_downloads_seconds = res.json()['aggregations']['downloads_timeframe']['downloads_per_second']['buckets']
  ...
  if calls % 100 == 0:
    ...
    if len(pp_downloads_seconds) > 0: # only parse query output if there were actually views/downloads to parse
    
      # loop through each person in output
      for x in range(len(pp_downloads_seconds)):
      
            # append new row with info for the preprint, the user, the timestamp, and the action count
        new_row = [{'guid':guid, 'date':pp_downloads_seconds[x]['key_as_string'],'download_count': pp_downloads_seconds[x]['doc_count']}]
